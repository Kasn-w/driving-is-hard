...
# T-junctions: visual only - you drive straight through, nothing to turn onto.
# Each entry is just the world-z where a cross street crosses your road.
@export var junction_width: float = 1800.0   # how thick the cross street is, along z
@export var junction_span: float = 9000.0    # recycle distance once passed
var junctions: Array = [4200.0]
...
func _ready() -> void:
	...
	cars.append(Car.new(2600.0, -360.0, Color(0.91, 0.30, 0.24), 280.0, 200.0))
...
func road_center_x(z: float) -> float:
	return sin(z * 0.0009) * 1300.0 + sin(z * 0.00033) * 2000.0
...
func project(world_x: float, world_y: float, world_z: float) -> Dictionary:
	...
	var scale = focal / dz
	var cam_x = road_center_x(cam_z) + player_offset
	return {
...
# True if the [z0, z1] road slice overlaps any junction's cross-street band.
func _in_junction(z0: float, z1: float) -> bool:
	var half = junction_width / 2.0
	for jz in junctions:
		if z1 >= jz - half and z0 <= jz + half:
			return true
	return false
...
func _process(delta: float) -> void:
	...
	var max_off = road_width * 1.3
	player_offset = clamp(player_offset, -max_off, max_off)
	...
	for i in range(junctions.size()):
		if junctions[i] < cam_z - 100.0:
			junctions[i] += junction_span
...
func _draw() -> void:
	...
	while n >= 1:
		...
		var grass_col: Color
		if _in_junction(z0, z1):
			grass_col = Color(0.40, 0.40, 0.43)
		else:
			grass_col = Color(0.23, 0.56, 0.23) if seg_index % 2 == 0 else Color(0.20, 0.52, 0.23)
...
func _draw_junction_edge(edge_z: float) -> void:
	var half_thick = 25.0
	var far = project(0.0, 0.0, edge_z + half_thick)
	var near = project(0.0, 0.0, edge_z - half_thick)
	if far.dz <= 1.0 or near.dz <= 1.0:
		return
	draw_rect(Rect2(0, far.y, screen_w, max(1.0, near.y - far.y)), Color(0.92, 0.85, 0.25))
